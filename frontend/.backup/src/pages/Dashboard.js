...
import { userService, gamificationService, aiService } from '../services/api';
...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setStats(prev => ({
        ...prev,
        totalPoints: pointsResponse.data.total_points || 1250,
        level: pointsResponse.data.current_level || 'Intermediate'
      }));

      // Load recent activities  
      const activitiesResponse = await userService.getActivities(userId);
      setRecentActivity(activitiesResponse.data.activities || []);
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button className="p-4 bg-blue-50 hover:bg-blue-100 rounded-lg text-center transition-colors">
              <BrainCircuitIcon className="w-8 h-8 text-blue-600 mx-auto mb-2" />
            ...
            <button className="p-4 bg-yellow-50 hover:bg-yellow-100 rounded-lg text-center transition-colors">
              <AwardIcon className="w-8 h-8 text-yellow-600 mx-auto mb-2" />
