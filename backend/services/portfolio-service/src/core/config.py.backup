"""
🔒 SECURE CONFIGURATION - PORTFOLIO SERVICE
...
class Settings(BaseSettings):
    """
    Secure configuration for Portfolio Service
    Uses environment variables instead of hardcoded values
    """
    
    # Service Configuration
    service_name: str = Field(default="portfolio-service", env="SERVICE_NAME")
    service_host: str = Field(default="0.0.0.0", env="SERVICE_HOST")
    service_port: int = Field(default=5005, env="PORTFOLIO_PORT")
    ...
    # 📁 PORTFOLIO SPECIFIC
    portfolio_templates_enabled: bool = Field(default=True, env="PORTFOLIO_TEMPLATES_ENABLED")
    content_generation_enabled: bool = Field(default=True, env="CONTENT_GENERATION_ENABLED")
    static_assets_path: str = Field(default="/app/static", env="STATIC_ASSETS_PATH")
    max_portfolio_size_mb: int = Field(default=50, env="MAX_PORTFOLIO_SIZE_MB")
    
    # Export Settings
    pdf_export_enabled: bool = Field(default=True, env="PDF_EXPORT_ENABLED")
    html_export_enabled: bool = Field(default=True, env="HTML_EXPORT_ENABLED")
    template_cache_ttl: int = Field(default=7200, env="TEMPLATE_CACHE_TTL")  # 2 hours
...
try:
    ...
    print("✅ Portfolio Service: All environment variables validated successfully")
except ValueError as e:
    print(f"❌ Portfolio Service: {e}")
